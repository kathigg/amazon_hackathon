...
- **Images**: real-world only, no AI-generated bill art, no portraits. The current system is a curated pool. Pipeline order: `npm run curate:images` populates `BillImageAsset` from Wikimedia Commons + Openverse (CC0/PDM only — strict license filtering at the source) into S3 → `npm run embed:images` fills `BillImageAsset.embedding` (Titan Multimodal v1, 1024-dim); without this, every cosine pick falls back to deterministic hash → `npm run reassign:images -- --apply` writes `Bill.imageAssetId` using cosine(`Bill.topicEmbedding`, asset embedding) with a global 1-to-1 uniqueness invariant, falling back to a deterministic hash when no embedding is available. `npm run backfill:bill-images` (alias for `scripts/backfill-bill-image-assets.ts`) is the per-bill helper used during ingest. `npm run backfill:images` (legacy `scripts/backfill-bill-images.ts`) only writes the deprecated `Bill.imageUrl` hotlink — keep it for the fallback chain but don't use it for new assignments. Runtime read order: `Bill.imageAsset.cdnUrl` → legacy `Bill.imageUrl` → static topic-pool fallback. Don't fetch, generate, or upload images at request time.
- **Image-assignment invariant**: any script that mutates `Bill.topicTags`, `Bill.legislativeSubjects`, or re-fetches Congress.gov metadata MUST end with `assignBillImageAsset(billId, topicTags, legislativeSubjects)` (and `ensureTopicEmbedding(billId, title, subjects)` if `topicEmbedding` is empty), because `imageAssetId` is keyed off those exact fields. Summary-only refreshers (`backfill:summaries`, `regenerate-summary.ts`) are exempt — the embedding corpus is `[title, ...subjects]`, not the summary. After `purge:pdf-images --apply`, always run `reassign:images -- --apply`.
